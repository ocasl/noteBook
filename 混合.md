...
```tsx
$ npm init   // 可以自动生成package.json文件  项目名字和版本号
```
...
- `"files"`指定一个包含相对或绝对文件路径的列表。 `"include"`和`"exclude"`属性指定一个文件全局匹配模式列表
...
```ts
Math.round 四舍五入   abs 绝对值     ceil(天花板)/flool(地板)  上/下取整  max min 取大小值  sqrt 开平方  pow 取幂 
...
<img src="混合.assets/image-20211023155231646.png" alt="image-20211023155231646"  />

**Attenion**：是没有insertAfter的  
...
![image-20211023164219777](混合.assets/image-20211023164219777.png)
...
![image-20211023172830096](混合.assets/image-20211023172830096.png)

需要传两个参数   代码执行一次    方法名，时间 ，时间不写默认立即开始(但是加入到了异步队列肯定比同步慢)   第三个可以给方法传参
